...
resource "azurerm_private_endpoint" "fucntion_private_endpoint" {
...
resource "azurerm_private_dns_a_record" "function_private_dns_a_record" {
    ...
    resource_group_name = azurerm_resource_group.rg.name
    ttl                 = 300
    records             = [azurerm_private_endpoint.fucntion_private_endpoint.private_service_connection[0].private_ip_address]
  
}

resource "azurerm_private_dns_zone_virtual_network_link" "function_vnet_link" {
    name                = "functionvnet-${var.project}-${var.environment}"
    resource_group_name = azurerm_resource_group.rg.name
